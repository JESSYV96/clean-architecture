...
import { appActions, initialState, quizzSlice } from '../../type'
...
it('should be able to use this only for multiple answers', () => {
  ...
  const newState = quizzSlice.reducer(
    state,
    appActions.removeIncorrectAnswers({ question: booleanQuestion, randomNumber: Math.random() })
  )
...
it('should not be able to used this joker if it does not remain joker', () => {
  ...
  const newState = quizzSlice.reducer(
    state,
    appActions.removeIncorrectAnswers({ question, randomNumber: Math.random() })
  )
  const finalState = quizzSlice.reducer(
    newState,
    appActions.removeIncorrectAnswers({ question, randomNumber: Math.random() })
  )
